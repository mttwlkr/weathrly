...
const CurrentWeather = (props) => {
  const {currentHumidity, currentLocation, currentTemp_F, currentWeatherSummary, currentWindMPH, precipTodayIn, icon_url, currentDate, icon, currentExplanation, low, high} = props.weather;
  const base = 'https://i.imgur.com/'
  return (
    <div className="mainDiv">
      <div className="locationDiv">
        <h3 className="currentLocation">{currentLocation}</h3>
        <h3 className="currentDate">{currentDate}</h3>
      </div>
      <h3 className="temp">CURRENT</h3>
      <h3 className="tempData">{currentTemp_F}&#8457;</h3>

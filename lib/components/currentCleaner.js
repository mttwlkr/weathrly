export const currentCleaner = (data) => {
  ...
  const explanation = 
    data.forecast.txt_forecast.forecastday.reduce((string, day) => {
      if (day.period === 0) {
        string = day.fcttext;
      }
      return string;
  ...
  const high = 
    data.forecast.simpleforecast.forecastday.reduce((string, day) => {
      if (day.period === 1) {
  ...
  const currentDate = 
    observation.split(' ').slice(1, 3).reverse().toString().replace(/,/g, ' ');

  const lastUpdate = data.current_observation.observation_time_rfc822; 
  const timeOnly = 
    lastUpdate.split('').slice(17, 19).toString().replace(/,/g, '');
  let day = false;
